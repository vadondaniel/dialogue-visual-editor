...
class DialogueBlockWidgetLazyEditorTests(unittest.TestCase):
    ...
    def test_editor_mode_shows_inferred_line1_speaker_in_meta(self) -> None:
        segment = _segment([r"\C[2]\N[1]\C[0]", "こんにちは"])
        ...
        self.assertEqual(widget._speaker_display_name(), r"\C[2]\N[1]\C[0] (line 1)")
        self.assertIn("(line 1)", widget._speaker_display_name_html())
        self.assertIn("Speaker:", widget.meta_label.text())
        self.assertIn("(line 1)", widget.meta_label.text())
        widget.deleteLater()
    ...
    def test_translator_mode_source_hint_can_use_shared_source_resolver(self) -> None:
        ...
        widget.source_hint_lines_resolver = lambda _segment: ["Shared source line"]
        widget.set_editor_active(True)
        self._app.processEvents()
        ...
        assert overlay is not None
        self.assertFalse(overlay.isHidden())
        self.assertIn("Shared source line", overlay.text())
        widget.deleteLater()

    def test_translator_mode_control_mismatch_can_use_chain_resolvers(self) -> None:
        ...
        widget.control_mismatch_source_lines_resolver = (
            lambda _segment: [r"\C[14]Press\C[0]"]
        )
        widget.control_mismatch_translation_lines_resolver = (
            lambda _segment: [r"\C[14]Press", r"\C[0]"]
        )
    ...
    def test_control_mismatch_highlight_uses_local_text_even_with_chain_resolver(self) -> None:
        ...
        widget.set_control_mismatch_highlighting_enabled(True)
        widget.control_mismatch_source_lines_resolver = (
            lambda _segment: ["Source text"]
        )
        widget.control_mismatch_translation_lines_resolver = (
            lambda _segment: ["Translated text"]
        )
